...
filename=/media/tjbieber/OS/testCopy
...
for i in {1..20}
	do
		cp /media/tjbieber/OS/testCopy "/media/tjbieber/OS/testCopy$i"
	done
# remove these test files and time.
for i in {1..20}
	do
		...
		# get the start time.
		t1=$(date +%s%3N)
		./rm "/media/tjbieber/OS/testCopy$i"
		sync
		# get the finish time.
		t2=$(date +%s%3N)
		# get the time used.
...
filename=/media/tjbieber/OS/testFolder
...
for i in {1..20}
	do
		...
		# get the start time.
		t1=$(date +%s%3N)
		./rm -r "/media/tjbieber/OS/testFolder$i"
		sync
		# get the end time.
		t2=$(date +%s%3N)
		# get the time used.
